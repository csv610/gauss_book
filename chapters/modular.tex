...
Carl Friedrich Gauss revolutionized number theory with his seminal work \textit{Disquisitiones Arithmeticae} (1801), where he systematized the theory of congruences \index{congruence}. The notion of modular arithmetic \index{modular arithmetic}---working with remainders upon division---provides the foundation for modern cryptography, coding theory, and computational number theory.

In this chapter, we explore:
\begin{itemize}
    \item Congruences and residue classes \index{residue class}
    \item Modular arithmetic operations
    \item The Chinese Remainder Theorem \index{Chinese Remainder Theorem} (Gauss's proof)
    \item Primitive roots and discrete logarithm \index{discrete logarithm}s
...
Let $n_1, n_2, \ldots, n_k$ be pairwise coprime \index{coprimality} positive integers. For any integers $a_1, a_2, \ldots, a_k$, the system of congruences
\[
\begin{cases}
...
Let $p$ be an odd prime. An integer $g$ is a \textit{primitive root modulo $p$} \index{primitive root} if the order of $g$ modulo $p$ is $p-1$, i.e., the smallest positive integer $k$ such that $g^k \equiv 1 \pmod{p}$ is $k = p-1$.
\end{definition}

Gauss proved that primitive roots exist \index{primitive root!existence} for all prime moduli (Article 55). More generally, primitive roots exist modulo $n$ iff $n = 2, 4, p^k, 2p^k$ for an odd prime $p$.
...
Gauss's \textit{Disquisitiones Arithmeticae} was written when he was just 21 years old (published at 24). The work introduced the notation $a \equiv b \pmod{n}$ and systematically developed the theory of congruences \index{congruence}. Article 36--44 contains the Chinese Remainder Theorem with Gauss's own proof. Articles 52--60 establish the existence of primitive roots for prime moduli. The discrete logarithm (index) calculus in Articles 81--85 was used by Gauss for extensive numerical computations.
