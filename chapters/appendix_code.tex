...
\texttt{prime\_counting.py} & Sieve of Eratosthenes, $\pi(x)$, $x/\log x$ and $Li(x)$ estimates \\
...
\texttt{geodesy.py} & Triangulation, resection, spherical excess, network adjustment \\
...
\texttt{hyperbolic.py} & Poincar\'e models, hyperbolic distance, angle of parallelism \\
...
\begin{lstlisting}[style=python]
from python.prime_counting import prime_pi, nth_prime, gauss_estimate

# pi(x) and the n-th prime
print(prime_pi(10**6))          # 78498
print(nth_prime(1000))          # 7919

# Gauss's estimate x/log x vs. pi(x)
print(gauss_estimate(10**6))    # 72382.4
\end{lstlisting}

\subsection{Chapter 8: AGM and \texorpdfstring{$\pi$}{Pi}}
\begin{lstlisting}[style=python]
...
\subsection{Chapter 9: Gaussian Elimination}
\begin{lstlisting}[style=python]
...
\begin{lstlisting}[style=python]
from python.geodesy import forward_triangulation, resection, spherical_excess
from math import radians

# Baseline triangulation: apex of an equilateral triangle
pt, d1, d2 = forward_triangulation((0, 0), (1, 0), radians(60), radians(60))

# Three-point problem
alpha, beta = radians(56.3099), radians(75.9638)
P = resection((0, 0), (1, 0), (2, 1), alpha, beta)  # (0.7, 0.9)

# Spherical excess of the octant triangle is 90 degrees
e, area = spherical_excess(0, 0, 0, 90, 90, 0)
\end{lstlisting}

\subsection{Chapter 15: Heptadecagon}
\begin{lstlisting}[style=python]
...
\begin{lstlisting}[style=python]
from python.hyperbolic import (half_plane_distance, half_plane_to_disk,
                               disk_distance, angle_of_parallelism)

# Distances agree in both models of the hyperbolic plane
z1, z2 = 0.5j, 0.3 + 0.7j
print(half_plane_distance(z1, z2))                 # 0.600391
print(disk_distance(half_plane_to_disk(z1), half_plane_to_disk(z2)))

# Lobachevsky's angle of parallelism Pi(1) = 2 arctan(e^{-1})
print(angle_of_parallelism(1.0))                   # 0.7050
\end{lstlisting}
