...
\begin{itemize}
    \item The Legendre symbol and Euler's criterion \index{Euler's criterion}
    \item Gauss's Lemma \index{Gauss's Lemma} and the first proof of quadratic reciprocity
    \item The Jacobi and Kronecker symbol \index{Kronecker symbol}s
    \item Algorithms for computing square roots modulo primes (Tonelli-Shanks \index{Tonelli-Shanks algorithm}, Cipolla)
...
Let $p$ be an odd prime and $a \in \Z$. The \textit{Legendre symbol} \index{Legendre symbol} is defined as
\[
\legendre{a}{p} = 
\begin{cases}
0 & \text{if } p \mid a, \\
1 & \text{if } a \text{ is a quadratic residue \index{quadratic residue} modulo } p, \\
-1 & \text{if } a \text{ is a quadratic non-residue \index{quadratic non-residue} modulo } p.
\end{cases}
\]
\end{definition}
...
\begin{supplement}[First Supplement \index{quadratic reciprocity!supplements}]
For odd prime $p$,
\[
...
The Jacobi symbol \index{Jacobi symbol} extends the Legendre symbol to composite odd denominators.
...
cipolla(a, p)                   # Cipolla's algorithm \index{Cipolla's algorithm}
...
\begin{exercise}
Implement the Solovay-Strassen primality test using the Jacobi symbol and Euler's criterion. Test on known primes and Carmichael numbers \index{Carmichael number}.
\end{exercise}
