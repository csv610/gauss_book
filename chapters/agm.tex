...
The AGM has deep connections to:
\begin{itemize}
    \item Elliptic integrals and modular forms \index{modular form}
    \item Fast algorithms for computing $\pi$ \index{pi!computational algorithms}
    \item Theta functions \index{theta function} and their transformations
    \item The theory of quadratic forms \index{quadratic form}
\end{itemize}
...
\begin{definition}[Complete Elliptic Integral of the First Kind]\index{elliptic integral!complete}
\[
K(k) = \int_0^{\pi/2} \frac{d\theta}{\sqrt{1 - k^2 \sin^2\theta}} = \int_0^1 \frac{dt}{\sqrt{(1-t^2)(1-k^2 t^2)}}, \quad 0 \le k < 1.
\]
\end{definition}

\begin{theorem}[Gauss's Identity]\index{Gauss's identity}
\[
...
\begin{definition}[Gauss's Constant]\index{Gauss's constant}
\[
G = \frac{1}{M(1, \sqrt{2})} = \frac{2}{\pi} \int_0^{\pi/2} \frac{d\theta}{\sqrt{1+\sin^2\theta}} \approx 0.8346268...
\]
\end{definition}

\begin{theorem}[Lemniscate Constant]\index{lemniscate constant}
The lemniscate constant $\varpi = 2 \int_0^1 \frac{dt}{\sqrt{1-t^4}}$ satisfies $\varpi = \frac{\Gamma(1/4)^2}{2\sqrt{2\pi}} = 2M(1, \sqrt{2})$.
\end{theorem}
...
\begin{itemize}
    ...
    \item \cite{brent-salamin,salamin-1976} -- Brent (1976) \& Salamin (1976) algorithms for $\pi$
\end{itemize}