...
Gaussian quadrature is a numerical integration method that achieves exact results for polynomials up to degree $2n-1$ using only $n$ function evaluations. Gauss developed this method in his work on the arithmetic-geometric mean and the theory of special functions.

In this chapter, we cover:
\begin{itemize}
    \item Orthogonal polynomials and their zeros
    \item Gauss-Legendre quadrature
    \item Gauss-Hermite, Gauss-Laguerre, Gauss-Chebyshev quadrature
    \item The Golub-Welsch algorithm for computing nodes and weights
...
Let $w(x) > 0$ be a weight function on $[a,b]$. The monic orthogonal polynomials $p_n(x)$ satisfy
\[
\int_a^b p_n(x) p_m(x) w(x) dx = 0 \quad (n \ne m).
\]
They obey a three-term recurrence:
\[
...
gauss_kronrod(n)            # Kronrod extension for error estimate
\end{lstlisting}
