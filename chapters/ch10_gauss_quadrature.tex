...
\begin{exercise}
Prove that Gaussian quadrature achieves exactness for polynomials of degree $2n-1$.
\end{exercise}
...
\begin{solution}
For the proof of exactness, any polynomial $f$ of degree $\le 2n-1$ can be written as $f(x) = q(x)p_n(x) + r(x)$ where $\deg(q) \le n-1$ and $\deg(r) \le n-1$. By orthogonality, $\int f(x)w(x)dx = \int r(x)w(x)dx$. Since the quadrature is exact for polynomials of degree $< n$, and the nodes are chosen as zeros of $p_n$, the rule gives exact results.
\end{solution}
